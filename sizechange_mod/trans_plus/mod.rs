...
pub fn global_fighter_frame(fighter : &mut L2CFighterCommon) {
    unsafe {
        ...
        if FighterUtil::is_scaling(module_accessor) == false {
            ...
            // change size
            /*
            if SCALE[entry_id] <= SS[entry_id] * 0.9546 {
            ...
            else if SCALE[entry_id] >= SS[entry_id] * 1.0473 {
                ShieldModule::set_attack_mul(module_accessor, 1.0 / (SCALE[entry_id] * BASE05[entry_id].powf(CT05[entry_id]) * BASE01[entry_id].powf(CT01[entry_id])), *FIGHTER_SHIELD_KIND_GUARD);
                smash::app::sv_battle_object::fixed_scaling(fighter_id, true, *FIGHTER_SCALING_KIND_KINOKO, *FIGHTER_SCALING_TYPE_BIG, SCALE[entry_id], *FIGHTER_SCALING_STATUS_BIG);
                smash::app::sv_battle_object::fixed_scaling(fighter_id, true, *FIGHTER_SCALING_KIND_KINOKO, *FIGHTER_SCALING_TYPE_BIG, SCALE[entry_id], *FIGHTER_SCALING_STATUS_BIG);
                CT2[entry_id] = 0.0;
                CT10[entry_id] = 0.0;
