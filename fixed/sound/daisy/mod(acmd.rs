...
/*
#[acmd_script( agent = "daisy", script = "game_appealsl", category = ACMD_GAME )]
unsafe fn daisy_appealsl(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=16)
        if(is_excute){
            ATTACK(ID=0, Part=0, Bone=hash40("kneer"), Damage=8.0, Angle=361, KBG=102, FKB=0, BKB=55, Size=5.0, X=6.0, Y=1.0, Z=0.0, X2=LUA_VOID, Y2=LUA_VOID, Z2=LUA_VOID, Hitlag=1.0, SDI=1.0, Clang_Rebound=ATTACK_SETOFF_KIND_ON, FacingRestrict=ATTACK_LR_CHECK_F, SetWeight=false, ShieldDamage=0, Trip=0.0, Rehit=0, Reflectable=false, Absorbable=false, Flinchless=false, DisableHitlag=false, Direct_Hitbox=true, Ground_or_Air=COLLISION_SITUATION_MASK_GA, Hitbits=COLLISION_CATEGORY_MASK_ALL, CollisionPart=COLLISION_PART_MASK_ALL, FriendlyFire=false, Effect=hash40("collision_attr_cutup"), SFXLevel=ATTACK_SOUND_LEVEL_L, SFXType=COLLISION_SOUND_ATTR_KICK, Type=ATTACK_REGION_KICK)
        ...
        frame(Frame=43)
        if(is_excute){
            ATTACK(ID=0, Part=0, Bone=hash40("kneel"), Damage=8.0, Angle=151, KBG=102, FKB=0, BKB=55, Size=5.0, X=6.0, Y=1.0, Z=0.0, X2=LUA_VOID, Y2=LUA_VOID, Z2=LUA_VOID, Hitlag=1.0, SDI=1.0, Clang_Rebound=ATTACK_SETOFF_KIND_ON, FacingRestrict=ATTACK_LR_CHECK_F, SetWeight=false, ShieldDamage=0, Trip=0.0, Rehit=0, Reflectable=false, Absorbable=false, Flinchless=false, DisableHitlag=false, Direct_Hitbox=true, Ground_or_Air=COLLISION_SITUATION_MASK_GA, Hitbits=COLLISION_CATEGORY_MASK_ALL, CollisionPart=COLLISION_PART_MASK_ALL, FriendlyFire=false, Effect=hash40("collision_attr_cutup"), SFXLevel=ATTACK_SOUND_LEVEL_L, SFXType=COLLISION_SOUND_ATTR_KICK, Type=ATTACK_REGION_KICK)
            CameraModule::req_quake(*CAMERA_QUAKE_KIND_KL)
        }
        frame(Frame=47)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "game_appealsr", category = ACMD_GAME )]
unsafe fn daisy_appealsr(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=16)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_squat", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_squat(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_squat"))
        }
        frame(Frame=3)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
        }
    });
}

#[acmd_script( agent = "daisy", script = "sound_attack11", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundattack11(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=1)
        if(is_excute){
        ...
        frame(Frame=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_swing_s"))
        }
        frame(Frame=38)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
#[acmd_script( agent = "daisy", script = "sound_attack12", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundattack12(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=2)
        if(is_excute){
...
unsafe fn daisy_soundutilt(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=9)
        if(is_excute){
        ...
        wait(Frames=1)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_attackhard_h02"))
        }
        frame(Frame=29)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
#[acmd_script( agent = "daisy", script = "sound_attacks3", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundstilt(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=6)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_attacks4hi", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundssmashhi(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=11)
        if(is_excute){
        ...
        wait(Frames=1)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_attack05"))
        }
        wait(Frames=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_s02"))
        }
        frame(Frame=45)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_left_s"))
        }
    });
}

#[acmd_script( agent = "daisy", script = "sound_attacks4", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundssmashs(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=11)
        if(is_excute){
        ...
        wait(Frames=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_s01"))
        }
        frame(Frame=45)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_left_s"))
...
#[acmd_script( agent = "daisy", script = "sound_attacks4lw", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundssmashlw(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=11)
        if(is_excute){
        ...
        wait(Frames=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_s03"))
        }
        frame(Frame=45)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_left_s"))
...
#[acmd_script( agent = "daisy", script = "sound_attackhi4", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundusmash(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=11)
        if(is_excute){
        ...
        wait(Frames=1)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_attack06"))
        }
        wait(Frames=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_h01"))
        }
        frame(Frame=33)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
unsafe fn daisy_sounddsmash(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=4)
        if(is_excute){
        ...
        wait(Frames=1)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_attack07"))
        }
        wait(Frames=1)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_l01"))
        }
        wait(Frames=5)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_l02"))
        }
        wait(Frames=5)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_smash_l02"))
...
unsafe fn daisy_soundappealhil(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=12)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_appeal_l01"))
        }
        frame(Frame=15)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_appeal_h01"))
        }
        frame(Frame=43)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
#[acmd_script( agent = "daisy", script = "sound_appealhir", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundappealhir(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=12)
        if(is_excute){
...
unsafe fn daisy_soundappeallwl(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=5)
        if(is_excute){
        ...
        frame(Frame=19)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_wear01"))
        }
        frame(Frame=20)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_left_s"))
        ...
        frame(Frame=21)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_appeal_l01"))
        }
        frame(Frame=68)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
#[acmd_script( agent = "daisy", script = "sound_appeallwr", category = ACMD_SOUND , low_priority )]
unsafe fn daisy_soundappeallwr(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=5)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_appealsl", category = ACMD_SOUND, low_priority )]
unsafe fn daisy_soundappealsl(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=12)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_appeal_s01"))
        }
        frame(Frame=18)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_wear02"))
        }
        frame(Frame=32)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
        ...
        frame(Frame=33)
        if(is_excute){
            PLAY_SE(hash40("vc_daisy_appeal_s01"))
        }
        frame(Frame=79)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_left_s"))
...
unsafe fn daisy_soundappealsr(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=12)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_catch", category = ACMD_SOUND, low_priority )]
unsafe fn daisy_soundcatch(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=6)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_throwhi", category = ACMD_SOUND, low_priority )]
unsafe fn daisy_soundthrowhi(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=2)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_throwb", category = ACMD_SOUND, low_priority )]
unsafe fn daisy_soundthrowb(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=2)
        if(is_excute){
...
#[acmd_script( agent = "daisy", script = "sound_specialsend", category = ACMD_SOUND, low_priority )]
unsafe fn daisy_soundspecialslanding(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    acmd!(lua_state, {
        frame(Frame=2)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_special_s04"))
        }
        frame(Frame=30)
        if(is_excute){
            PLAY_SE(hash40("se_daisy_step_right_s"))
...
pub fn install() {
    ...
    smashline::install_acmd_scripts!(
        ...
        daisy_soundssmashlw,
        daisy_sounddsmash,
        daisy_soundappealhil,
        daisy_soundappealhir,
        daisy_soundappeallwl,
        daisy_soundappeallwr,
        daisy_soundappealsl,
        daisy_soundappealsr,
        daisy_soundcatch,
        daisy_soundthrowhi,
        daisy_soundthrowb,
        daisy_soundspecialslanding
        */
    );
}
